...
def get_programs(site=None, uuid=None, uuids=None, course=None, organization=None):
    ...
    if uuid:
        ...
        uuids = cache.get(ORGANIZATION_PROGRAMS_CACHE_KEY_TPL.format(org_key=organization))
        if not uuids:
            return []
...
def get_programs_for_organization(organization):
    ...
    """
    return cache.get(ORGANIZATION_PROGRAMS_CACHE_KEY_TPL.format(org_key=organization))
