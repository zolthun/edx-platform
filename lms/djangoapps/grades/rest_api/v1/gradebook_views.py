...
class GradebookView(GradeViewMixin, PaginatedAPIView):
    ...
    **Paginated GET response**
            ...
            if request.GET.get('assignment') and (
                ...
                subqueryset = PersistentSubsectionGrade.objects.annotate(
                    effective_grade=Case(
                        When(override__isnull=False, then='override__earned_all_override'),
                        default='earned_all'))
                grade_conditions = {}
                if request.GET.get('assignment_grade_max'):
                    grade_conditions['effective_grade__lte'] = request.GET.get('assignment_grade_max')
                if request.GET.get('assignment_grade_min'):
                    grade_conditions['effective_grade__gte'] = request.GET.get('assignment_grade_min')
    ...
    def _get_user_count(self, query_args, cache_time=3600, annotations=None):
        ...
        if annotations:
            for key, val in six.iteritems(annotations):
                hasher.update(bytes(key))
                hasher.update(bytes(val))
        ...
        if user_count is None:
            ...
            user_count = queryset.filter(*query_args).count()
